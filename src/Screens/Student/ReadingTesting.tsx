...
import { RootStackParamList, useNavigationHelper } from '../../Controller/NavigationController';
import { AlphabetItem, Passage, isAlphabet, isPassage } from '../../Types/passage';
import readingStyles from '../../ui/ReadingActivityStyles';
import bubbles from '../../ui/BubblesDesign';
import selection from '../../ui/PassageSelectionStyles';
...
export default function ReadingActivityScreenPage() {
  ...
  useEffect(() => {
    if (spokenText && spokenText !== 'No speech detected') {
      const textToCompare = isAlphabet(readingMaterial) 
        ? readingMaterial.letter 
        : readingMaterial.text;
      const detectedMiscues = MiscueAnalysisService.detectMiscues(textToCompare, spokenText);
      setMiscues(detectedMiscues);
    }
  }, [spokenText, readingMaterial]);
  ...
  const handleRecordToggle = async () => {
    if (isRecording) {
      ...
      const textToRead = isAlphabet(readingMaterial) 
        ? readingMaterial.letter 
        : readingMaterial.text;
      await startRecording(textToRead);
    }
  };
  ...
  const handleAudioProcessing = async (audioFile: string) => {
    try {
      ...
      const textToCompare = isAlphabet(readingMaterial) 
        ? readingMaterial.letter 
        : readingMaterial.text;
      const detectedMiscues = MiscueAnalysisService.detectMiscues(textToCompare, transcription);
      setMiscues(detectedMiscues);

      const title = isAlphabet(readingMaterial) 
        ? `Alphabet: ${readingMaterial.letter}` 
        : readingMaterial.title;
      await MiscueReportController.storeReport(title, detectedMiscues);
      ...
      const textToCompare = isAlphabet(readingMaterial) 
        ? readingMaterial.letter 
        : readingMaterial.text;
      const simulatedResponse = getSimulatedResponse(textToCompare);
      setSpokenText(simulatedResponse);
      setIsReadingCompleted(true);
    }
  };
  ...
  return (
    <SafeAreaView style={readingStyles.container}>
      <View style={readingStyles.insideContainer}>
        {/* Bubble Decorations */}
        <View style={bubbles.bubblesContainer} pointerEvents="none">
          {/* Your existing bubble decorations */}
        </View>
        ...
        {!isLoading && !isRecording && isReadingCompleted && (
          <ReadingFeedback
            passageText={isAlphabet(readingMaterial) ? readingMaterial.letter : readingMaterial.text}
            spokenText={spokenText}
            miscues={miscues}
            onTryAgain={handleTryAgain}
          />
