...
const calculateWeightedMetric = (
  ...
  key: 'accuracyRate' | 'wordPerMin' | 'miscueDensity',
): number => {
...
const analyzeTrend = (
  ...
  const avgFirst =
    firstHalf.reduce((sum, r) => sum + r.accuracyRate, 0) / firstHalf.length;
  const avgSecond =
    secondHalf.reduce((sum, r) => sum + r.accuracyRate, 0) / secondHalf.length;
...
const calculateMiscueDensity = (
  report: MiscueReportDocument,
): number | null => {
...
const classifyStudent = (
  ...
  const avgLength =
    passageLengths.reduce((a, b) => a + b, 0) / (passageLengths.length || 1);

  // Scoring logic
  const accuracyScore = avgAccuracy;
  const wpmScore = Math.min(
    100,
    (avgWPM / SCORING_CONFIG.fluentWPM(gradeLevel)) * 100,
  );
  ...
  return {
    ...
    classificationScore: Number(
      Math.max(0, Math.min(100, finalScore)).toFixed(1),
    ),
    confidence:
      reports.length > 3 ? 'high' : reports.length > 1 ? 'medium' : 'low',
    ...
    lastReportDate:
      reports.length > 0
        ? reports[reports.length - 1].timestamp?.toDate?.() || new Date()
        : new Date(),
  };
};
    ...
    for (const classItem of classes) {
      ...
      const studentDataPromises = studentIds.map(async id => {
        ...
        const status = classifyStudent(
          reports,
          sData.studentData?.gradeLevel || 1,
        );
      ...
      const allResults = (await Promise.all(studentDataPromises)).filter(
        r => r !== null,
      ) as { status: StudentReadingStatus; hasParticipated: boolean }[];
      ...
      const categories = {
        ...
        insufficientData: allStatuses.filter(
          s => s.status === 'insufficientData',
        ),
      };

      const total = studentIds.length;
      const participatedCount = allResults.filter(
        r => r.hasParticipated,
      ).length;
      const participationRate =
        total > 0 ? (participatedCount / total) * 100 : 0;

      // Generate Required Actions based on class state
      let action = 'Continue regular assessments.';
      if (participationRate < 60)
        action = 'Urgent: Increase student participation in assessments.';
      else if (categories.atRisk.length > total * 0.2)
        action =
          'High alert: Consider small-group remedial reading intervention.';
      else if (categories.emerging.length > total * 0.3)
        action = 'Focus on oral reading fluency and accuracy drills.';
...
const calculateConsistency = (
  ...
  const variance =
    accuracies.reduce((a, b) => a + Math.pow(b - mean, 2), 0) /
    accuracies.length;
