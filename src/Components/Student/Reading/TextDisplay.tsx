...
export const PassageDisplay: React.FC<PassageDisplayProps> = ({
  ...
  // Shows uppercase and lowercase bubbles side by side with
  // a matching blue palette and a "Say this letter!" prompt.
  // ─────────────────────────────────────────────────────────
  if (isAlphabet(material)) {
    const upper = material.letter.toUpperCase();
    const lower = material.letter.toLowerCase();

    return (
      <View style={readingStyles.alphaDisplayCard}>
        {/* Letter bubbles */}
        <View style={readingStyles.alphaLetterRow}>
          {/* Uppercase — bigger bubble */}
          <View style={readingStyles.alphaUpperBubble}>
            <Text style={readingStyles.alphaUpperLetter}>{upper}</Text>
          </View>

          {/* Lowercase — smaller bubble */}
          <View style={readingStyles.alphaLowerBubble}>
            <Text style={readingStyles.alphaLowerLetter}>{lower}</Text>
          </View>
        </View>

        {/* Case labels */}
        <View style={readingStyles.alphaCaseLabelRow}>
          <Text style={[readingStyles.alphaCaseLabel, { width: 118 }]}>
            Uppercase
          </Text>
          <Text style={[readingStyles.alphaCaseLabel, { width: 84 }]}>
            Lowercase
          </Text>
        </View>

        {/* Divider */}
        <View style={readingStyles.alphaDivider} />

        {/* Instruction */}
        <Text style={readingStyles.alphaInstruction}>
          🔊 Say this letter!
        </Text>
      </View>
    );
  ...
  // Clean card with a "READ THIS WORD" label, the target word
  // large and centred, and a teal accent underline.
  // ─────────────────────────────────────────────────────────
  if (type === 'word' && isWords(material)) {
    const allWords = material.contrasts.flatMap(c => c.words);

    return (
      <View style={readingStyles.wordDisplayCard}>
        {/* Label */}
        <Text style={readingStyles.wordDisplayLabel}>Read this word</Text>

        {/* The word */}
        <Text style={readingStyles.wordDisplayText}>{allWords[0]}</Text>

        {/* Decorative accent underline */}
        <View style={readingStyles.wordAccentLine} />
      </View>
    );
  }

  // ─────────────────────────────────────────────────────────
  // PASSAGE DISPLAY — UNCHANGED
  // ─────────────────────────────────────────────────────────
  return (
    <View style={readingStyles.insideContainer}>
      {!isRecording && isReadingCompleted && type === 'passage' && (
