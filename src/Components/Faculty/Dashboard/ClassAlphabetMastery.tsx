...
import { useClassAlphabetMastery } from '../../../Hooks/Faculty/useClassAlphabetMastery';
...
const EmptySlotNotice = ({ label }: { label: string }) => (
  <View style={styles.emptyNotice}>
    ...
    <Text style={styles.emptyBody}>
      No class alphabet reading was recorded in this period. Select another period to
      view data.
    </Text>
  </View>
);
...
export default function ClassAlphabetMastery({ facultyId, filter }: Props) {
  const [timeRange, setTimeRange] = useState<TimeRange>('week');

  // Firestore + computed slots (accuracy + mastery + trend-ready)
  const { slots, summary, defaultIndex, loading, error } =
    useClassAlphabetMastery(facultyId, filter, timeRange);
    ...
    <View style={styles.mainCard}>
      ...
      <View style={styles.header}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.compSubtitle}>Class Average · {rangeLabel}</Text>
        </View>
