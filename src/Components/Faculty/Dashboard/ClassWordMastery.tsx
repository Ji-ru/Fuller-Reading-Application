...
export default function ClassWordMastery({ facultyId, filter }: Props) {
  const [timeRange, setTimeRange] = useState<TimeRange>('week');
  const { chapters, loading, error } = useClassWordMastery(facultyId, filter, timeRange);
    ...
    <View style={styles.mainCard}>
      ...
      <View style={styles.header}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.compSubtitle}>Class Average · Chapters · Lessons · Words</Text>
        </View>
      ...
      {currentChapter ? (
        ...
        <View style={styles.emptyNotice}>
          ...
          <Text style={styles.emptyBody}>
            Complete a word reading session to see your class progress here.
          </Text>
        </View>
      )}
